...
# from Cultural_Heritage import CulturalDestination
...
class TestCulturalDestination(unittest.TestCase):
    def setUp(self):
        self.my_cultural_destination = CulturalDestination("Heritage Arts", "Delhi")
    ...
    def test_add_art(self):
        self.my_cultural_destination.add_art("Mughal Miniature", "Visual Arts", "Paintings made during the Mughal Empire", "Bishandas")
        self.assertIn("Mughal Miniature", [art.name for art in self.my_cultural_destination.art_list])

    def test_remove_art(self):
        self.my_cultural_destination.remove_art("Bharatanatyam Dance")
        self.assertNotIn("Bharatanatyam Dance", [art.name for art in self.my_cultural_destination.art_list])

    def test_display_art(self):
        expected_output = "Heritage Arts collection:\n- Taj Mahal (Architecture) by Ustad Ahmad Lahauri: The iconic symbol of love in Agra\n- Bharatanatyam Dance (Performing Arts) by Rukmini Devi Arundale: Classical Indian dance form\n- Pattachitra Painting (Visual Arts) by Raghurajpur Artists: Traditional cloth-based scroll painting from Odisha\n"
        self.assertEqual(expected_output, self.my_cultural_destination.display_art())

    def test_get_random_art(self):
        random_art = self.my_cultural_destination.get_random_art()
        self.assertIn(random_art, self.my_cultural_destination.art_list)
